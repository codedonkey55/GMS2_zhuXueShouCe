...
- if判断：经过实践，发现可以不写小括号，像python那样。
- switch判断：适合3个以上的if，增加代码可读性。详见关键字部分。
- while循环：while(条件)
- do循环：先进行一次无条件循环，然后看条件是否满足，不满足继续循环。详见关键字。
- for循环：for(用于计数的变量 ; 条件 ; 每次循环后操作)
- repeat循环：是GML方便游戏设计自制的。repeat(循环次数)

其他说明：其他语言的break和continue一样可以使用。
...
- with( )。括号里指出一个实例，然后在大括号内可以暂时操控这个实例。括号里可以写实例，也可以写对象。区别就是写对象会影响所有它的实例。

```c
with(wanJia_id)
{
    代码区。这里的代码就像是在wanJia里写代码一样。
}
with(other) //这个other可用于碰撞事件里的代码，other指的就是碰到的那个实例。以及with子弹后朝向等于other就是等于发射者的朝向。
{
    
}
```

- #region和#endregion。折叠代码块。
- switch判断

```c
     switch (chengji)
     {
        case "A":show_message("80~90分！"); break;//千万别忘了加break，不然就会执行BCDEF的，不管逻辑
        case "B":show_message("70~80分！"); break;
        case "C":show_message("60~70分！"); break;
        case "D":show_message("50~60分！"); break;
        case "E":show_message("40~50分！"); break;
        case "F":show_message("30~40分！"); break;
        default: show_message("0~30分！"); 
        //case就是else if，default是else。
     }
```
...
之前说过自带的game_save函数。ini是最普遍的开发者的存档解决方案。过程受到高度控制因此安全些，存什么，读取什么，读取后如何应用。

这里要注意的是如果在存档文件中没有获取到对应的值时，会将此处第三个参数的内容作为默认值写入存档文件，切记。
...
找到defaults文件夹，用记事本打开里面的.json，可以看到字体和字号。如果要修改，建议使用微软雅黑字体，调高点字号，默认有点小。
...
手册中很多地方，写着大概、可能、具体详见文档。这些地方都是明显需要维护的。如有能力，请帮忙维护一下。当然有极个别的详见文档是因为内容太多，文档机翻质量够看，贴上了不简洁，所以就指向文档了。对比各种源码和官方文档，也能很清楚的方向本文档在哪些地方缺乏侧重性，就去补充上。
